...
test('the real game-params schema accepts the committed run', () => {
  ...
  // Two contracts are known to postdate the committed run, and exactly those are expected to
  // fail until the crew is re-recorded live. Anything else is a regression.
  //
  //   * the parachute contract (stated area and drag coefficient)
  //   * the one-band contract — GDD §2.6 is a single envelope with a value gradient, so the
  //     ablation maps are keyed by sample point (bottom/middle/top) rather than by band, and
  //     economy.band_value_multiplier became economy.value_gradient
  //
  // Do NOT silence these by hand-editing out/config/game_params.json. That file is the record
  // of what a run produced; migrating it would make it claim to be something it is not. It is
  // regenerated by the next live run, and these allowances come out then.
  //   * commit_floor_m — the rule the endgame rests on, added after this run was recorded
  const knownStale = /parachute_area_m2|parachute_drag_coefficient|heat_index|cost_curve|optimal_skims|value_gradient|commit_floor_m/;
  const unexpected = errs.filter((e) => !knownStale.test(e));
